...
reset:
    ...
    LDI TMP, 0xE7
    OUT DDRA, TMP       ; PA0-PA3 выход (разряды), PA4-PA5 вход (кнопки 8-9), PA6-PA7 выход (LED)
    LDI TMP, 0xDF
    OUT DDRD, TMP       ; PD5 выход, остальное вход (PD3=INT1)
    SER TMP
    OUT DDRC, TMP       ; PORTC выход (сегменты)

    LDI TMP, 0x0F
    OUT MCUCR, TMP      ; INT0, INT1 по фронту
    LDI TMP, 0xC0
    OUT GICR, TMP
    OUT GIFR, TMP
    SEI
    ...
    OUT TCCR0, TMP      ; таймер T0, прескалер 64
    LDI TMP, 131
    OUT TCNT0, TMP
    LDI TMP, (1<<TOIE0)
    OUT TIMSK, TMP
...
read_number:
    ; если уже введена хоть одна цифра — проверить таймаут
    TST  CURRENT_CELL
    BREQ RN_WAIT
    LDS  TMP,  TIMEOUT_CNT
    LDS  TMP2, TIMEOUT_CNT+1
    OR   TMP,  TMP2
    BREQ soft_reset     ; таймаут истёк
