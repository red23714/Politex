...
complete -c qemu-system-i386 -o fdb -d 'Use file as floppy disk 0/1 image (see the Disk Images \\%<#\\:disk-images> cha…'
...
complete -c qemu-system-i386 -o netdev -d 'Configure a passt network backend which requires no administrator privilege t…'
complete -c qemu-system-i386 -o net -d 'Legacy option to configure or create an on-board (or machine default) Network…'
complete -c qemu-system-i386 -o chardev -d 'Backend is one of: null, socket, udp, msmouse, hub, vc, ringbuf, file, pipe, …'
...
complete -c qemu-system-i386 -o shim -d 'Use \'shim. efi\' to boot the kernel'
...
complete -c qemu-system-i386 -o gdb -d 'Accept a gdb connection on device dev (see the GDB usage \\%<#\\:gdb-usage> cha…'
...
complete -c qemu-system-i386 -o semihosting -d 'Enable Semihosting \\%<#\\:semihosting> mode (ARM, M68K, Xtensa, MIPS, RISC-V o…'
complete -c qemu-system-i386 -o semihosting-config -d 'Enable and configure Semihosting \\%<#\\:semihosting> (ARM, M68K, Xtensa, MIPS,…'
