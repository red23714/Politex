...
complete -c grub-install -l appended-signature-size -d 'Add a note segment reserving SIZE bytes for an appended signature'
...
complete -c grub-install -s x -l x509key -d 'embed FILE as an x509 certificate for appended signature checking'
