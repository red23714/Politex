...
complete -c pahole -l padding -d 'Show only structs with SIZE_PADDING bytes of padding at its end'
complete -c pahole -l padding_ge -d 'Show only structs with at least SIZE_PADDING bytes of padding at its end'
complete -c pahole -s I -l show_decl_info -d 'Show the file and line number where the tags were defined, if available in th…'
complete -c pahole -l skip_encoding_btf_vars -d 'By default, VARs are encoded only for percpu variables'
...
complete -c pahole -l btf_features -d 'Encode BTF using the specified feature list, or specify \'default\' for all sta…'
...
complete -c pahole -l running_kernel_vmlinux -d 'Search for, possibly getting from a debuginfo server, a vmlinux matching the …'
...
complete -c pahole -l with_embedded_flexible_array -d 'Show only structs that have an embedded flexible array, i. e'
