...
complete -c docker-container-run -l use-api-socket -d '	Bind mount Docker API socket and required auth'
...
complete -c docker-container-run -s w -l workdir -d '	Working directory inside the container SEE ALSO docker-container(1)'
