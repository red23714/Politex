...
complete -c grub-mkimage -s k -l pubkey -d 'embed FILE as public key for PGP signature checking'
...
complete -c grub-mkimage -s S -l appended-signature-size -d 'Add a note segment reserving SIZE bytes for an appended signature'
complete -c grub-mkimage -s v -l verbose -d 'print verbose messages'
complete -c grub-mkimage -s x -l x509key -d 'embed FILE as an x509 certificate for appended signature checking'
