# systemd-measure
# Autogenerated from man page /usr/share/man/man1/systemd-measure.1.gz
complete -c systemd-measure -l linux -l osrel -l cmdline -l initrd -l ucode -l splash -l dtb -l uname -l sbat -l pcrpkey -l profile -l dtbauto -l hwids -d 'When used with the calculate or sign verb, configures the files to read the u…'
complete -c systemd-measure -l current -d 'When used with the calculate or sign verb, takes the PCR 11 values currently …'
complete -c systemd-measure -l bank -d 'Controls the PCR banks to pre-calculate the PCR values for \\(en in case calcu…'
complete -c systemd-measure -l private-key -l public-key -l certificate -d 'These switches take paths to a pair of PEM encoded RSA key files, for use wit…'
complete -c systemd-measure -l private-key-source -l certificate-source -d 'As an alternative to --public-key= for the sign command, these switches can b…'
complete -c systemd-measure -l tpm2-device -d 'Controls which TPM2 device to use'
complete -c systemd-measure -l phase -d 'Controls which boot phases to calculate expected PCR 11 values for'
complete -c systemd-measure -l append -d 'When generating a PCR JSON signature (via the sign command), combine it with …'
complete -c systemd-measure -l json -d 'Shows output formatted as JSON'
complete -c systemd-measure -l no-pager -d 'Do not pipe output into a pager'
complete -c systemd-measure -s h -l help -d 'Print a short help text and exit'
complete -c systemd-measure -l version -d 'Print a short version string and exit'
complete -c systemd-measure -l pcr-private-key -d 'option, covering all boot phases.  The "'
complete -c systemd-measure -l tpm2-public-key -d 'on the command line of those tools.  EXIT STATUS'

