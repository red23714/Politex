...
complete -c ctest -l resource-spec-file -d 'Run CTest with resource allocation enabled, using the resource specification …'
...
complete -c ctest -s S -d 'OUTPUT_JUNIT keyword with the ctest_test() %<#command:ctest_test> command ins…'
...
complete -c ctest -l interactive-debug-mode -d 'Disable (0) or enable (1) interactive debug mode'
...
complete -c ctest -l force-new-ctest-process -d Ignored
complete -c ctest -l schedule-random -d 'Use a random order for scheduling tests'
complete -c ctest -l schedule-random-seed -d 'Added in version 4. 1.  Override the random order seed'
...
complete -c ctest -l build-noclean
...
complete -c ctest -l test-command -d 'The command to run as the test step with the'
...
complete -c ctest -l script -d 'Execute a dashboard for a configuration'
...
complete -c ctest -l extra-submit -d 'Submit extra . xml part files to the dashboard'
