...
complete -c sg_modes -s A -l ALL -d 'same meaning as using the --all option twice'
...
complete -c sg_modes -s e -l examine -d 'if the --page=PG[,SPG] option is not given, examine each mode page in the ran…'
...
complete -c sg_modes -s H -l hex -d 'Since this utility shows the structure of a MODE SENSE command response (i. e'
...
complete -c sg_modes -s M -l multiple -d 'for each mode page (and mode subpage), fetch all available page controls'
