...
complete -c has160-hash -s u -l update -d 'Update the specified hash file by computing message digests for files listed …'
complete -c has160-hash -l missing -d 'List files from the hash file that are missing or inaccessible in the filesys…'
complete -c has160-hash -l unverified -d 'List command-line files missing from the given hash file (unverified files)'
...
complete -c has160-hash -l blake3 -d 'BLAKE3: Select BLAKE3 hash function'
...
complete -c has160-hash -l one-hash -d 'Print one message digest per line without additional file information'
complete -c has160-hash -s L -l ed2k-link -d 'Print eDonkey link containing file name, file size, ED2K and AICH message dig…'
