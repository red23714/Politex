...
complete -c sysctl -s w -l write -d 'Force all arguments to be write arguments and print an error if they cannot b…'
complete -c sysctl -s p -l load -d 'Load in sysctl settings from the file specified or /etc/sysctl'
complete -c sysctl -s a -l all -d 'Display all values currently available except deprecated and verboten paramet…'
...
complete -c sysctl -s V -l version -d 'Display version information and exit.  FILES /run/sysctl. d/*'
