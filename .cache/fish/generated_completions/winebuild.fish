...
complete -c winebuild -l disable-dynamicbase -d 'Disable \'ASLR\' address space layout randomization in executable image'
...
complete -c winebuild -o march -o mcpu -o mfpu -d 'Set code generation options for the assembler'
...
complete -c winebuild -l strip-cmd -d 'Specify the command to use to strip PE modules'
