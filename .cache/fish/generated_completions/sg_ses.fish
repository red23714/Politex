...
complete -c sg_ses -s a -l all -d 'shows the output of the --join option followed by the other SES dpages'
complete -c sg_ses -s z -l ALL -d 'shows the output of the --join option invoked twice, followed by the other SE…'
...
complete -c sg_ses -s J -l json -d 'output is in JSON format instead of plain text form'
complete -c sg_ses -s Q -l js-file -d 'output is in JSON format and it is sent to a file named JFN'
...
complete -c sg_ses -s F -l no-config -d 'the Element status, Array status (obsolete), Element Descriptor, Additional e…'
complete -c sg_ses -s y -l no-time -d 'this utility now issues a SCSI REPORT TIMESTAMP command just after an INQUIRY…'
...
complete -c sg_ses -o HHHHH -d 'To force the issue use -H times (as discouragement).  Instead using the'
complete -c sg_ses -o HHH -o HHHH
complete -c sg_ses -s w -l warn -d 'warn about certain irregularities with warnings sent to stderr'
complete -c sg_ses -s 1
complete -c sg_ses -o 1-1
