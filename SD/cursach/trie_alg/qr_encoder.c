...
int generate_qr(char* original_string, char* password){ 
    ...
    int key = generate_xor(password); 

    for(int i = 0; i < strlen(original_string); i++)
    {
        original_string[i] ^= key; 
    }
    ...
    // Сохраняем отдельные каналы для отладки
    combine_color_channels(red_channel, red_channel, red_channel, "red_channel.ppm");
    combine_color_channels(green_channel, green_channel, green_channel, "green_channel.ppm");
    combine_color_channels(blue_channel, blue_channel, blue_channel, "blue_channel.ppm");
