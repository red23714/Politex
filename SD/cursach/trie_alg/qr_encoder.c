...
void split_string_into_three_equal_parts(const char* input_string, char** part1,
										 char** part2, char** part3)
{
...
RGB** create_qr_code_mask(const char* data, ImageSize* size)
{
	...
	printf("QR %dx%d modules -> %dx%d pixels (scale: %d)\n", qr_module_size,
		   qr_module_size, size->width, size->height, scale);
	...
	for (int i = 0; i < size->height; i++)
	{
		mask[i] =
			calloc(size->width,
				   sizeof(RGB)); // Используем calloc для нулевой инициализации
	}
...
void combine_color_channels(RGB** red_channel, RGB** green_channel,
							RGB** blue_channel, const char* output_path,
							ImageSize size)
{
...
int generate_qr(char* original_string, char* password)
{
	...
	split_string_into_three_equal_parts(original_string, &data1, &data2,
										&data3);
	...
	combine_color_channels(red_channel, green_channel, blue_channel, "qr.ppm",
						   final_size);
